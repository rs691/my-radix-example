...
const FramerLayout = ({ children }) => {
  ...
  // Transforms based on scrollY
  const translateY1 = useTransform(scrollY, [0, 500], [0, 50]);
  const translateY2 = useTransform(scrollY, [0, 500], [0, 100]);
  const translateY3 = useTransform(scrollY, [0, 500], [0, 150]);

  // Listen for scroll events
  useEffect(() => {
    const handleScroll = () => {
      scrollY.set(window.scrollY); // Update scrollY value as you scroll
    };
  ...
  return (
    <>
      ...
      {/* Main Content */}
      <div
        className={`min-h-screen bg-cover bg-center bg-fixed bg-no-repeat transition-colors duration-300 relative z-10 ${
